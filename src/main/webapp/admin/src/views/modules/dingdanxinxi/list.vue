<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','申请售后') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="shouhouCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'申请售后':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','申请售后') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="shouhouCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'申请售后':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','申请售后') && contents.tableBtnIcon == 0" type="success" size="mini" @click="shouhouCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'申请售后':'' }}</el-button>
    ...
    <shouhou-cross-add-or-update v-if="shouhouCrossAddOrUpdateFlag" :parent="this" ref="shouhouCrossaddOrUpdate"></shouhou-cross-add-or-update>
...
import shouhouCrossAddOrUpdate from "../shouhou/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      shouhouCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    shouhouCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.shouhouCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
      this.$refs.shouhouCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        ...
        //  shuliang
        // xcolumn ycolumn shangpinmingcheng shuliang
        var shuliangChart = this.$echarts.init(document.getElementById("shuliangChart"),'macarons');
        this.$http({
            url: `value/dingdanxinxi/shangpinmingcheng/shuliang`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
